...
channels = 3
classes = 11
train_np = np.load(MODEL_DATASET_PATH + "train.npy")
test_np = np.load(MODEL_DATASET_PATH + "test.npy")
train_temp = [0]
test_temp = [0]
...
def inference_op(input_op, keep_prob):
    ...
    fc3 = fc_op(fc2_drop, name='fc3', n_out=classes, p=p)
...
def run_benchmark():
    with tf.Graph().as_default():
        ...
        for i in range(num_batches + num_steps_burn_in):
            ...
            if i >= num_steps_burn_in:
                if not (i - num_steps_burn_in) % 10:
                    ...
                    print('%s: step %d / %d, duration = %.3f, loss = %.8f, training accuracy %g' %
                          (datetime.now(), i - num_steps_burn_in, num_batches, duration, _loss, train_accuracy))
                    f.write("%d %.8f %g\n" % (i - num_steps_burn_in, _loss, train_accuracy))
        ...
        saver.save(sess, RESULT_MODEL_RESULT_PATH + MODEL_NAME)

        out_put = 0
        for _ in range(len(test_np)):
            batch = next_batch(1, test_temp, test_np)
            feed_test_set = {x: batch[0].reshape([-1, image_height, image_width, channels]),
...
f = open(RESULT_PATH + "trained_result.txt", "w")
